...
Raid é sistema de [[CyberSecurity/fatec/2024/1-2024/dspti/backup]] apenas para disponibilidade composto por uma controladora RAID e a partir de 2 discos, não serve de [[CyberSecurity/fatec/2024/1-2024/dspti/backup]] de [[Segurança]]. O ideal é ter 2 discos de marcas diferentes;
...
RAID1 | Mirroring => Redundância Mesmo hardware do Raid0 O arqivo é escrito em todos [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] discos, tendo uma cópia do mesmo arquivo em ttodos A velocidade de leitura é identica, mas a de escrita é maior (o dobro) O armazenamento torna-se menor, pois precisa-se reservar [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] dois discos para

Comparativo Raids

Raid0 | Raid1 Vw => 1s | Vw => 2s Vr => 1s | Vr => 1s Sp => 2T | Sp => Sp

Parity Disk Considere que temos 4 data disks, onde cada um deles possui 1T de armazenamento funcionando em RAID 1, mas o ultimo disco funciona apenas como Parity Disk, ou seja, nele será gravado o [[CyberSecurity/fatec/2024/1-2024/dspti/backup]] de tudo o que for escrito, enquanto nos demais a gravação será sequencial. Para identificação dos discos temos identificações baseadas em bits setados (1) e resetados (0). Supondo que um disco falhe e precisemos encontrar qual o id de nossas identificações de disco que falhou basta fazer um xor nos ids sobresalentes que o resultado é o id do que falhou:
...
Comparando [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] bits mais significativos (o bit mais à esquerda, ou seja, o que vale 128b) faz com que criemos o id de nosso parity disk
...
Portanto se tivermos 8 discos podemos ter até 3 pontos de falha A partir daqui podemos perder até 2 discos, antes poderia-se perder apenas 1 RAID1+0 Mais eficiente Combinação entre o RAID1 (duplica [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] dados entre dois discos fisicos) e RAID0 (fragmenta entre 2 discos lógicos) RAID0+1 Menos eficiente Combinação dentre o RAID0 (fragmenta [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] dados entre [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] discos físicos) e RAID1 (duplica [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] dados entre [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] discos lógicos)
