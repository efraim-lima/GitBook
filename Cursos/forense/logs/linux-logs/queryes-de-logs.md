...
O processo de análise de [[CyberSecurity/Course/forense/logs/logs]] infere que precisamos reconstruir uma cadeia de eventos com o objetivo de se compreender a linha de tempo do que ocorreu e ter uma melhor visibilidade de como pode ter ocorrido um incidente, levantando cada etapa do incidente seja antes ou depois do mesmo ocorrer. Portanto é importante saber correlacionar [[CyberSecurity/fatec/Pesquisa/Pesquisa/ferramentas/os/os|os]] eventos detectados com etapas de um incidente, isso trará clareza e consciência de cada etapa e também a possiblidade de inferir qual foi a intenção da ação do atacante no ambiente.&#x20;

A partir disso é interessante correlacionar o framework [[CYBER]] Kill Chain, que descreve as etapas de um ataque cibernético e queryes especificas que podemos inputar no [[CyberSecurity/Course/tools/operational-systems/linux/linux|linux]] que levantarão eventos que podem ser associados a cada etapa do incidente.&#x20;

Abaixo está uma lista detalhada dos IDs de eventos relevantes para o [[CyberSecurity/Course/tools/operational-systems/windows/windows|windows]], categorizados por cada etapa da [[CYBER]] Kill Chain.\


Queries de Análise de [[CyberSecurity/Course/forense/logs/logs]] para Análise [[CyberSecurity/Course/forense/forense|forense]] em Cada Etapa do [[CYBER]] Kill Chain
...
* **Query**: Verificar [[CyberSecurity/Course/forense/logs/logs]] de acesso a arquivos críticos.
...
* Query: Verificar [[CyberSecurity/Course/forense/logs/logs]] de e-mail indicando e-mails suspeitos com anexos.
...
* **Query**: Analisar [[CyberSecurity/Course/forense/logs/logs]] de escalonamento de privilégios.
...
* **Query**: Verificar [[CyberSecurity/Course/forense/logs/logs]] de exportação ou cópia de dados.
...
* **Query**: Analisar [[CyberSecurity/Course/forense/logs/logs]] de execução de [[scripts]] ou [[comandos]] remotos.
  * **Comando**: `grep "bash" /var/log/auth.log`

#### 6. Ação

* **Query**: Identificar [[CyberSecurity/Course/forense/logs/logs]] relacionados a ações maliciosas, como exclusão de arquivos.
  * **Comando**: `grep "delete" /var/log/audit/audit.log`
* **Query**: Monitorar [[CyberSecurity/Course/forense/logs/logs]] de serviços críticos que foram paralisados ou reiniciados.
...
* **Query**: Analisar [[CyberSecurity/Course/forense/logs/logs]] de tráfego de rede para detectar padrões anômalos.
  * **Comando**: `tcpdump -i eth0 port not 22`

Essas queries são fundamentais para a análise [[CyberSecurity/Course/forense/forense|forense]] em sistemas [[CyberSecurity/Course/tools/operational-systems/linux/linux|linux]], permitindo identificar e investigar [[Work/To Do/Under/Atividades/Atividades]] suspeitas em cada etapa do [[CYBER]] Kill Chain, contribuindo para uma resposta rápida e eficaz a incidentes de [[Segurança]].\
\
Referências:\
